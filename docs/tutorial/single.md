...
|  |  |
|--|--|
| CPU.sv | CPU の実装．基本的にはこの CPU モジュール以下に実装をおこなっていきます．|
| DMem.sv | データメモリ．このファイルは基本的には書き換えません．|
| DMem.dat | データメモリの中身．ソートの入力データなどをここに記述します．|
| IMem.sv | 命令メモリ．このファイルは基本的には書き換えません．|
| IMem.dat | 命令メモリの中身．命令コードをここに記述します．|
| IOCtrl.sv | I/Oインターフェースの実装．このファイルは書き換えません．|
| MainSim.sv | シミュレーション入力などの検証用コード．|
| Main.sv | メインモジュール．ここに CPU やデータメモリ，命令メモリなどを含みます．|
...
- データメモリへの書き込みについては，等速クロック（clk）の後半のみで行うよう気をつけてください（Main.sv 内のコメント参照）．
